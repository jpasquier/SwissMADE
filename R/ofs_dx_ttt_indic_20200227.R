...
simp_names <- excel_sheets(f) %>%
  {setNames(., .)} %>%
  lapply(function(z) read_xlsx(f, z))
rm(f0, f)

# xlsx sheets: simplified names
f0 <- "xlsx_sheet_names_simplified_MALP.xlsx"
f <- file.path("documents", f0)
if(!file.exists(f)) f <- tk_choose.files(caption = paste("Select", f0))
if (length(f) == 0) stop("No file selected")
if (length(f) > 1) stop("More than one file selected")
...
tbls <- mclapply(1:2, function(k) {
  ...
  code_list_1 <- code_tbl[[k]][[code]] %>%
    ...
    setNames(., .) %>%
    as.list()
  code_list_2 <- c("xlsx_sheet_simplified", paste0("supp_col_", 1:3)) %>%
    {code_tbl[[k]][.]} %>%
    as.matrix() %>%
    as.vector() %>%
    unique() %>%
    ...
    setNames(., .) %>%
    lapply(function(z) {
      sort(unique(unlist(lapply(0:3, function(j) {
        u <- if (j == 0) "xlsx_sheet_simplified" else paste0("supp_col_", j)
        b <- code_tbl[[k]][[u]] %in% z
        code_tbl[[k]][[code]][b]
      }))))
    })
...
# Save databases
out_dir <- "data/ofs_dx_ttt_indic_20200227"
...
sink(file.path(si_dir, "sessionInfo_20200227.txt"))
