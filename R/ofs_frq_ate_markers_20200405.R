...
frq_tbl <- mclapply(list(dx = ofs_dx, ttt = ofs_ttt), function(db) {
...
# Add inndicators to the results
frq_tbl <- append(frq_tbl,
                  list(dx_indicators = ofs_dx, ttt_indictors = ofs_ttt))

# Export frequencies
write_xlsx(frq_tbl, "results/ofs_frq_ate_markers_20200405.xlsx")
